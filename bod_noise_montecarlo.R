...
hia_age <- function(age = allages[5],
  ...
  popcity <- sofia_noise_build[, c("Poly_ID", age, pollutant)]
  names(popcity)[2] <- "pop"
...
allages <- names(sofia_noise_build)[c(10:19, 25:34)] # define age groups to analyze
res_noise_nat_mort <- hia_sim(pollutant = "noise",
                              outcome = "nat_mort",
                              nsim = 500, 
                              seed = 666)
sum(res_noise_nat_mort[res_noise_nat_mort$age == "overall", ]$mean) # 902 natural-cause deaths

# CVD mortality
allages <- names(sofia_noise_build)[c(10:19, 25:34)] # define age groups to analyze
res_noise_cvd_mort <- hia_sim(pollutant = "noise",
                              outcome = "cvd_mort",
                              nsim = 500, 
                              seed = 666)
sum(res_noise_cvd_mort[res_noise_cvd_mort$age == "overall", ]$mean) # 325 CVD deaths 

# Annoyance
allages <- "total_adults" # define age groups to analyze
res_noise_annoyance <- hia_sim(pollutant = "noise",
                               outcome = "annoyance",
                               nsim = 500, 
                               seed = 666)
summary(res_noise_annoyance[res_noise_annoyance$age == "overall", ]$mean) # mean 22.1% of population annoyed

# sleep disturbance
allages <- "total_adults" # define age groups to analyze
res_noise_sleep <- hia_sim(pollutant = "noise",
                           outcome = "sleep_dist",
                           nsim = 500, 
                           seed = 666)
summary(res_noise_sleep[res_noise_sleep$age == "overall", ]$mean) # mean 7.5% of population sleep disturbed
...
res_noise_ihd <- hia_sim(pollutant = "noise",
                         ...
                         nsim = 500, 
                         seed = 666)
sum(res_noise_ihd[res_noise_ihd$age == "overall", ]$mean) # 558 IHD cases 
...
res_noise_stroke <- hia_sim(pollutant = "noise",
                            ...
                            nsim = 500, 
                            seed = 666)
sum(res_noise_stroke[res_noise_stroke$age == "overall", ]$mean) # 329 stroke cases 
...
res_noise_diabetes <- hia_sim(pollutant = "noise",
                              ...
                              nsim = 500, 
                              seed = 666)
sum(res_noise_diabetes[res_noise_diabetes$age == "overall", ]$mean) # 621 diabetes cases


# Save the results
# Overall results by neighborhood
res_noise_rawlist <- list(res_noise_nat_mort, res_noise_cvd_mort, res_noise_annoyance, res_noise_sleep, res_noise_ihd, res_noise_stroke, res_noise_diabetes)
# Read census tracts
...
for (i in 1:length(res_noise_rawlist)) {
  ...
  if(unique(res$outcome) %in% c("sleep_dist", "annoyance")){
    
    # Export results
    res <- res[c("Poly_ID", "outcome", "mean", "pct2.5", "pct97.5")]
  ...
  # as csv
  write.csv(res, paste0("Baseline_BOD/Results/res_noise_", unique(res$outcome), ".csv"))
  # as Geojson
  res <- merge(sofia_poly, res, by.x = "Poly_ID", by.y = "Poly_ID")
  st_write(res, paste0("Baseline_BOD/Results/res_noise_", unique(res$outcome), ".geojson"))

}

# Overall results for the city
# Mortality outcomes
res_noise_rawlist <- list(res_noise_nat_mort, res_noise_cvd_mort)
res_noise_city_mort <- NULL # data frame to store results
...
for (i in 1:length(res_noise_rawlist)) {
  ...
  res_noise_city_mort <- rbind(res_noise_city_mort, res)
...
# Disease outcomes
res_noise_rawlist <- list(res_noise_annoyance, res_noise_sleep, res_noise_ihd, res_noise_stroke, res_noise_diabetes)
...
for (i in 1:length(res_noise_rawlist)) {
  ...
  if(unique(res$outcome) %in% c("annoyance", "sleep_dist")){
    
    res <- res %>% group_by(outcome, age) %>% summarise(mean = mean(mean, na.rm = T), pct2.5 = mean(pct2.5, na.rm = T), pct97.5 = mean(pct97.5, na.rm = T))
    res$age <- ifelse(res$age == "overall", "adults", res$age)
    res <- res %>% mutate(sex = "total", pop = sum(sofia_pop_poly$total_adults), rate = NA, exp_cases = NA, per_cases = NA, per_cases_lwr = NA, per_cases_upr = NA, att_rate = NA, att_rate_lwr = NA, att_rate_upr = NA)
    # Order results
    res <- res[c("outcome","age", "sex", "pop", "rate", "exp_cases", "mean", "pct2.5", "pct97.5", "per_cases", "per_cases_lwr", "per_cases_upr", "att_rate", "att_rate_lwr", "att_rate_upr")]
...
### Export the results
write_csv(res_noise_city_mort, "Baseline_BOD/Results/RESULTS_NOISE_mortality.csv")
write_csv(res_noise_city_disease, "Baseline_BOD/Results/RESULTS_NOISE_disease.csv")

# ### Plot the results
# # Mortality
res_noise_city_mort <- read_csv2("Baseline_BOD/Results/RESULTS_NOISE_mortality.csv")
res_noise_city_mort <- res_noise_city_mort[res_noise_city_mort$sex == "total", ]
res_noise_city_mort <- res_noise_city_mort[!res_noise_city_mort$age == "adults", ]

res_noise_city_mort$outcome <- factor(
  res_noise_city_mort$outcome,
...
theme_set(theme_bw())
ggplot(res_noise_city_mort, aes(x=age, y=mean)) + 
  geom_bar(stat = "identity", fill = "paleturquoise4") +
  facet_wrap(. ~ outcome, scales = "free") +
  coord_flip() + labs(title=expression("Noise attributable mortality by age group"),y ="Attributable deaths (n)", x = "") +
  geom_errorbar(aes(ymin = pct2.5, ymax = pct97.5), width = 0.2, color = "black")
ggsave("Baseline_BOD/Results/RESULTS_noise_age.png")
