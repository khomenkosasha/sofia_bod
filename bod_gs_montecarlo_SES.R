...
hia_age <- function(age = allages[19],
  ...
  if(outcome == "nat_mort"){
    ...
    # RR by SES group 
    rr_ses <- data.frame(ses_group = c("Low SES", "Medium-Low SES", "Medium-High SES", "High SES"), 
                         rr = c(0.95, 0.95, 0.94, 0.91), 
                         rrlo95 = c(0.94, 0.94, 0.93, 0.90), 
                         rrup95 = c(0.96, 0.97, 0.95, 0.93))
    scale <- 0.14
    
    # Then, we simulate a RR value assuming log(RR) is normal 
    rr_ses$selogrr <- log(rr_ses$rrup95 / rr_ses$rrlo95) / (2 * qnorm(0.975)) # we calculate the standard deviation of log(RR)
    rr_ses$logrr <- log(rr_ses$rr) # we calculate log(RR)
    rr_ses$logrrsim <- rnorm(n = nrow(rr_ses), mean = rr_ses$logrr, sd = rr_ses$selogrr) # we simulate one log(RR) value from a normal distribution
    rr_ses$rr <- exp(rr_ses$logrrsim) # We exponentiate back the simulated log(RR) --> the simulated RR will be then used in the HIA calculation
  ...
  if(outcome == "nat_mort"){
    ...
    mratecityage <- as.numeric(sofia_mort[sofia_mort$age == agesub & sofia_mort$sex == sexsub & sofia_mort$outcome == outcome, "rate"])
  ...
  if(outcome == "nat_mort"){
    ...
    popcity <- merge(popcity, rr_ses[c("ses_group", "rr")], by = "ses_group")
    popcity$rr <- exp((log(popcity$rr) / scale) * popcity$expdiff)
...
res_ndvi_nat_mort <- hia_sim(pollutant = "ndvi",
                             ...
                             nsim = 500, 
                             seed = 666)
sum(res_ndvi_nat_mort[res_ndvi_nat_mort$age == "overall", ]$mean) # 240 natural-cause deaths 
...
res_ndvi_hypertension <- hia_sim(pollutant = "ndvi",
                           ...
                           nsim = 500, 
                           seed = 666)
sum(res_ndvi_hypertension[res_ndvi_hypertension$age == "overall", ]$mean) # 257 hypertension cases 
...
res_ndvi_stroke <- hia_sim(pollutant = "ndvi",
                           ...
                           nsim = 500, 
                           seed = 666)
sum(res_ndvi_stroke[res_ndvi_stroke$age == "overall", ]$mean) # 45 stroke cases 


# Save the results
# Overall results by neighborhood
res_ndvi_rawlist <- list(res_ndvi_nat_mort, res_ndvi_hypertension, res_ndvi_stroke)
# Read census tracts
...
for (i in 1:length(res_ndvi_rawlist)) {
  ...
  # as csv
  write.csv(res, paste0("Baseline_BOD/Results/res_ndvi_SES_", unique(res$outcome), ".csv"))
  # as Geojson
  res <- merge(sofia_poly, res, by.x = "Poly_ID", by.y = "Poly_ID")
  st_write(res, paste0("Baseline_BOD/Results/res_ndvi_SES_", unique(res$outcome), ".geojson"))
...
### Export the results
write_csv(res_ndvi_city_disease, "Baseline_BOD/Results/RESULTS_NDVI_SES_disease.csv")
